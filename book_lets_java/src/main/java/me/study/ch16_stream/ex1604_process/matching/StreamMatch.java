package me.study.ch16_stream.ex1604_process.matching;
...
public class StreamMatch {
	public static void main (String[] args) {
		...
		// 요소들 중 하나 이상이 Rectangle 의 객체인지 조건 비교
		boolean result2 = list.stream().anyMatch(a -> (a instanceof Rectangle));
		System.out.println("요소들 중 Rectangle의 객체가 존재한다. -> " + result2);
...
/* 요소 조건 검사 */
// 스트림 내부의 요소들이 특정 조건을 만족하는지 검사할 수 있는 메서드

// allMatch(Predicate p)    요소들이 모두 p 조건 만족하면 true
// anyMatch(Predicate p)    요소들 중 하나라도 p 조건 만족하면 true
// noneMatch(Predicate p)   요소들이 모두 p 조건 만족하지 않으면 true
